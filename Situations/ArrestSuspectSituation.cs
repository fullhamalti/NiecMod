...
namespace NiecMod.CopMod
{
    public class ArrestSuspectSituation : RootSituation
    {
        ...
        public static ArrestSuspectSituation Create(Lot lot, Sim simToArrest, bool bUnsafe)
        {
            if (NiecMod.Nra.NFinalizeDeath.SC_GetObjects<IPoliceStation>().Length == 0)
            {
                return null;
            }
            if (!bUnsafe)
            {
                foreach (Situation sAllSituation in Situation.sAllSituations)
                {
                    if (sAllSituation is ArrestSuspectSituation && (sAllSituation as ArrestSuspectSituation).SimToArrest == simToArrest)
                    {
                        return null;
                    }
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (outfit != null)
                            ...
                            parent.Cop = description.Instantiate(Vector3.OutOfWorld);

                        parent.Cop.FlagSet(Sim.SimFlags.IgnoreDoorLocks, true); // Custom
                        parent.Cop.GreetSimOnLot(parent.ArrestLocation);
                    }
        ...
        private class MakeArrest : ChildSituation<ArrestSuspectSituation>
        {
            ...
            public override void Init(ArrestSuspectSituation parent)
            {
                if (base.Parent.SimToArrest != null)
                {
                    ...
                    if (instance != null)
                    {
                        parent.Cop.RequestWalkStyle(Sim.WalkStyle.ForceRun); // Custom
